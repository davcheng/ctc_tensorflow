...
def load_training_batch_data(input_path):
    ...
    # batch_train_targets = np.vstack(a for a in batch_train_targets)
    # batch_train_inputs = np.hstack(a for a in batch_train_inputs)
    # batch_train_inputs = np.asarray([ for t in timesteps])
    # print('shape') # (413, num_examples) if using batch_train_inputs.append
    # print(np.asarray(batch_train_targets).shape)
    # print('yo')
