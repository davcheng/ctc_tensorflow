...
INFERENCE_FILE = './timit_raw/DR1/FCJF0/SI1027.WAV'
FROZEN_GRAPH_FILE = './frozen_graph_20180405.pb'

def convert_wav_to_mfcc_inputs(wav_path):
    inference_fs, inference_audio = wav.read(wav_path)
...
def load_graph(frozen_graph_filename):
    ...
    with tf.Graph().as_default() as graph:
        ...
        tf.import_graph_def(graph_def, name="ctc")
    return graph

def main(_):
    print(FLAGS.frozen_model_filename)
    # Load graph via "load_graph" function
    graph = load_graph(FLAGS.frozen_model_filename)

    # Verify we have access to the list of operations in the graph
    for op in graph.get_operations():
        print(op.name)
        # prefix/Placeholder/inputs_placeholder...

    # Gather MFCC data
    wav_input, seq_len_input = convert_wav_to_mfcc_inputs(INFERENCE_FILE)
